...
- [x] Migración `017_event_kind_descanso.sql` aplicada el 21-08-2026. Es la que permite guardar un descanso. **Pendiente de revalidar** con `node scripts/validate-rls.mjs`.
- [ ] Migración `018_person_kind.sql` aplicada. **Pendiente**: añade `children.kind`, que es lo que permite dar de alta a un adulto sin cuenta (un abuelo). Sin ella, «Añadir adulto» en Ajustes falla porque la columna no existe. Tras aplicarla, revalidar con `node scripts/validate-rls.mjs`.
...
Resultados en **`docs/supabase-validation.md`**: 51/51 comprobaciones correctas, con las 16 primeras migraciones validadas (la 017 está aplicada y sin revalidar; la 018, sin aplicar). Repetible con `node scripts/validate-rls.mjs`.
