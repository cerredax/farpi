...
npm run test:unit      # 504 tests de lógica pura (~2 s, sin servidor)
npm run test:e2e       # suite completa: 673 (504 unitarios + 169 de navegador; levanta dev en :3100 en modo demo forzado)
...
- App Router. Rutas de app bajo el grupo `src/app/(app)/` (home, calendar, tasks, lists, meals, finanzas, notes, docs, birthdays, settings); auth en `src/app/auth/`; onboarding en `src/app/onboarding/`. Fuera del grupo y sin sesión: `/privacidad` y `/terminos` (públicas, y requisito para publicar en Google Play) y `/offline` (fallback del service worker).
